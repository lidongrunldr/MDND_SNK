...
class MatchingDataset(Dataset):
    def __init__(self, root_dir, train=False, k_eig=128, Nf=6, use_cache=True):
        super().__init__()
        ...
        self.classes_names=['cat','centaur','david','dog','horse','michael','victoria','wolf']
        ...
        if use_cache:
            ...
            if os.path.exists(load_cache):
                ...
                self.massvec_list, self.evals_list, self.evecs_list,\
                     self.gs_list,self.gradX_list, self.gradY_list, self.L_list, self.combinations, self.names_list,self.elevals_list,self.elevecs_list = torch.load(load_cache)
                return
            print("  --> dataset not in cache, repopulating")
        
        if self.train:
            self.root_dir=osp.join(root_dir,'train')
        else:
            self.root_dir=osp.join(root_dir,'test')
        
        files_name=os.listdir(osp.join(self.root_dir,'shapes'))
        
        # read file and process
        file_count=0
        for class_name in self.classes_names:
            # get all files in this class_name
            class_files_name=fnmatch.filter(files_name, class_name+'*.off')

            for file_name in class_files_name:
                name,_=osp.splitext(file_name)
                mesh_path=osp.join(self.root_dir,'shapes',name+'.off')
                ...
                # verts=diffusion_net.geometry.normalize_area_scale(verts,faces)

                self.verts_list.append(verts)
                self.faces_list.append(faces)
                self.names_list.append(name)
                elevals=sio.loadmat(osp.join("/home/mj/RFMNet/data/TOSCA/elevals",name+'.mat'))['elevals']
                self.elevals_list.append(torch.from_numpy(elevals))
                elevecs=sio.loadmat(osp.join("/home/mj/RFMNet/data/TOSCA/elevecs_zj",name+'.mat'))['elevecs_zj']
                self.elevecs_list.append(torch.from_numpy(elevecs))                
                file_count+=1
            
            self.combinations+=list(permutations(range(file_count-len(class_files_name),file_count),2))
        ...
        if use_cache:
            ...
            torch.save((self.verts_list, self.faces_list, self.descs_list, \
                self.massvec_list, self.evals_list, self.evecs_list,\
                     self.gs_list,self.gradX_list, self.gradY_list, self.L_list, self.combinations,self.names_list,self.elevals_list,self.elevecs_list), load_cache)
