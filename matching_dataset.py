...
class MatchingDataset(Dataset):
    def __init__(self,root_dir,train,k_eig=128,Nf=6,use_cache=True):
        ...
        if use_cache:
            ...
            if os.path.exists(load_cache):
                ...
                self.massvec_list, self.evals_list, self.evecs_list,\
                     self.gs_list,self.gradX_list, self.gradY_list, self.L_list,self.elevals_list,self.elevecs_list ,self.names_list= torch.load(load_cache)
        ...
        for name in self.names:
            ...
            elevals=sio.loadmat(osp.join("/home/mj/RFMNet/data/SCAPE/elevals",name+'.mat'))['elevals']
            self.elevals_list.append(torch.from_numpy(elevals))
            # elevecs=sio.loadmat(osp.join("/home/mj/RFMNet/data/SCAPE/elevecs_zj",name+'.mat'))['elevecs_zj']
            elevecs=sio.loadmat(osp.join("/home/mj/RFMNet/data/SCAPE/elevecs",name+'.mat'))['elevecs']
        ...
        # for s in range(len(self.elevals_list)):
        #     elevals=self.elevals_list[s]
        #     self.elgsonly_list=[]
        #     positive_evals = elevals[elevals > 0]
        #     negative_evals = elevals[elevals < 0]
        #     for i in range(len(elevals)):
        #         if elevals[i]>0:
        #             self.elgsonly_list.append(Meyer(positive_evals[-1],Nf=self.Nf)(eval))
        #         else elevals[i]<0:
        #             self.elgsonly_list.append(Meyer(negative_evals[-1],Nf=self.Nf)(eval)) 
        #     self.elgs_list.append(self.elgsonly_list)                           
        ...
        if use_cache:
            ...
            torch.save((self.verts_list, self.faces_list, self.descs_list, \
                self.massvec_list, self.evals_list, self.evecs_list,\
                     self.gs_list,self.gradX_list, self.gradY_list, self.L_list,self.elevals_list,self.elevecs_list,self.names_list), load_cache)
